                                        ...
                                        <tr id="hide{{ $item->id }}">
                                            <td class="text-left">
                                                @if (Auth::user()->hasRole('admin'))
                                                    <form
                                                        onsubmit="return confirm('Password akan direset berdasarkan usernamenya ( {{ $item->username ?? '' }} ), tekan ok untuk melanjutkan')"
                                                        action="{{ route('users.update', $item->id) }}" method="POST"
                                                        ...
                                                        <button type="submit"
                                                            ...
                                                            <i class="fa fa-refresh"></i> <span
                                                                class="pl-1 hidden-xs hidden-sm">Reset Password</span>
                                                        </button>
                                                    </form>
                                            ...
                                            <td>{{ $item->username ?? '' }}</td>
                                            <td><span
                                                    class="badge {{ $item->role->name == 'admin' ? 'badge-success' : 'badge-secondary' }}  text-lowercase">{{ $item->role->name }}</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
