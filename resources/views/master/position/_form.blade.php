    ...
    <div class="form-group row">
        ...
        <div class="col-12 col-md-5 col-lg-5">
            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                value="{{ old('name', $position->name ?? '') }}" placeholder="Name.." autocomplete="off">
            @error('name')
        ...
        <div class="col-8 col-md-4 col-lg-4">
            <div class="input-group">
                ...
                <input type="text" name="salary" class="form-control @error('salary') is-invalid @enderror"
                    value="{{ old('salary', $position->salary ?? '') }}" placeholder="100.000" autocomplete="off"
                    oninput="format(this)">
            </div>
            @error('salary')
                <span class="invalid-feedback" role="alert">
    ...
    <div class="form-group row">
        ...
        <div class="col-12 col-md-5 col-lg-5">
            <select name="status" class="form-control select2 @error('status') is-invalid @enderror">
                ...
                <option value="Staff" {{ 'Staff' == old('status', $position->status ?? '') ? 'selected' : '' }}>Staff
                </option>
                <option value="Daily Worker"
                    {{ 'Daily Worker' == old('status', $position->status ?? '') ? 'selected' : '' }}>Daily Worker
                </option>
            </select>
            @error('status')
                <span class="invalid-feedback" role="alert">
